          ...
          if(userEnteredValue.length >= minimumCharacter) {
            ...
            if ( sObjectType == 'Account' ) {
              ...
              for(var i = 0;i<comparisonField.length;i++) {
                  if(comparisonField[i] === 'ltimos_4_d_gitos_tarjeta__c'){
                     if( !isNaN(userEnteredValue) && userEnteredValue.length <= 4){
                        
                        //comparisionStringArray.push(comparisonField[i]+" LIKE '%*"+userEnteredValue+"%'");
                        //var comparisionString = comparisionStringArray.join(' OR ');
                        var comparisionString = " ID IN (SELECT Account__c FROM Tarjeta__c WHERE ltimos_4_d_gitos_tarjeta__c='"+userEnteredValue+"')";
